...
- **FRED "직전 회차" 값 재사용 방지(baseline)**: FRED는 발표 전에도 직전 회차 값을 계속 갖고 있어서, 그걸 그대로 가져다 쓰면 아직 발표 안 된(심지어 내일 발표할) 지표를 직전 값으로 미리 "발표"해버리는 문제가 있었습니다. 그래서 각 지표를 처음 확인할 때의 FRED 값을 `state.json`에 baseline으로 기록해두고, 그 이후 baseline과 다른(=진짜 새로 나온) 관측치가 나타날 때만 실측치로 채택합니다. 단, 봇을 오랫동안 멈춰뒀다가 이미 발표가 끝난 뒤에 처음 켜는 것처럼 baseline을 발표 "후" 값으로 잘못 기록하게 되는 극히 드문 경우에는, 다음 회차 전까지 그 지표의 결과 메시지가 나가지 않을 수 있습니다(평소처럼 폴링이 계속 돌고 있다면 거의 발생하지 않습니다).
